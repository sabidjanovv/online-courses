...
import { LessonsService } from '../../lessons/lessons.service'; // ← qo'shamiz

@Injectable()
export class LessonCreateUpdateGuard implements CanActivate {
  constructor(
    private readonly jwtService: JwtService,
    private readonly teachersService: TeachersService,
    private readonly coursesService: CoursesService,
    private readonly modulesService: ModulesService,
    private readonly lessonsService: LessonsService, // ← injektsiya qilamiz
  ) {}
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (req.method === 'POST') {
      ...
      if (!lessonId) {
        throw new ForbiddenException('Lesson ID talab qilinadi');
      }

      const lesson = await this.lessonsService.findOne(lessonId);
      if (!lesson) {
        throw new ForbiddenException('Lesson topilmadi');
      }

      moduleId =
        lesson.module_id?._id?.toString() || lesson.module_id?.toString();
      if (!moduleId) {
        throw new ForbiddenException('Lesson ichida module_id yo‘q');
      }
    } else {
